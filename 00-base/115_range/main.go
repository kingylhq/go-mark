...
func rangeStr() {
	...
	fmt.Printf("len(bytesFromStr2) = %d\n", len(bytesFromStr2)) // 12 一个中文字符占用三个字节
}

//这里需要强调一点，在 Go 中，所有字符串都是按照 Unicode 编码的。
//第一 for 循环中，遍历了变量 str1，它有六个字符，而且这些字符都可以使用一个 byte 表示，所以循环了六次才退出循环。
//而第二个 for 循环中，遍历了变量 str2，它有四个中文字符，那么按照 Unicode 编码的标准，很显然不能仅仅被四个 byte 表示，
//但它还是只循环了 4 次，恰好是中文字符的长度。 那么也就是说，在 Go 中，遍历字符串时，实际上是在遍历从字符串转换来的 rune 切片，
//只是恰好在某些时候，字符串转换成 byte 切片和字符串转换成 rune 切片之后的长度相同，看起来是在逐个遍历 byte 切一样。
